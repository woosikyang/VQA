...
def filter_answers(answers_dset, min_occurence):
    ...
    tmp_occurence = list(occurence.keys())
    for answer in tmp_occurence :
...
if __name__ == '__main__':
    ...
    train_answer_file = os.path.join(config.data_path,train_answer_file)
    train_answers = json.load(open(train_answer_file))['annotations']

    val_answer_file = 'v2_mscoco_val2014_annotations.json'
    val_answer_file = os.path.join(config.data_path,val_answer_file)
    val_answers = json.load(open(val_answer_file))['annotations']

    train_question_file = 'v2_OpenEnded_mscoco_train2014_questions.json'
    train_question_file = os.path.join(config.data_path,train_question_file)
    train_questions = json.load(open(train_question_file))['questions']

    val_question_file = 'v2_OpenEnded_mscoco_val2014_questions.json'
    val_question_file = os.path.join(config.data_path,val_question_file)
