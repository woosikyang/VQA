...
from dataset_test import VQAFeatureDatasettest
import base_model
...
if __name__ == '__main__':
    ...
    #test
    test_dset = VQAFeatureDatasettest('test',dictionary)
    ...
    model = getattr(base_model, constructor)(test_dset, 1024).cuda()
    ...
    ###
    #if test
    state_dict = torch.load('saved_models/exp4/model.pth')
    #load saved model
    ...
    logger = utils.Logger(os.path.join(output, 'log_0606.txt'))
    #for i, (v, b, q, a) in enumerate(test_loader):
    #q3 = np.empty([0, max_length])
    pred3 = np.empty([0, num_ans])
    for i, (v, b, q, a) in enumerate(tqdm(test_loader)):
        if i == 800 :
            break
        with torch.no_grad() :
            v = v.cuda()
    ...
    np.save('data/test_pred_answer_0606_0.npy',pred3)
    ...
    for i, (v, b, q, a) in enumerate(tqdm(test_loader)):
        if i >= 800 :
            ...
            with torch.no_grad() :
                ...
                #del q2
                del pred2
    np.save('data/test_pred_answer_0606_1.npy', pred3)
    ...
    test_pred_answer_0 = np.load('data/test_pred_answer_0606_0.npy')
    test_pred_answer_1 = np.load('data/test_pred_answer_0606_1.npy')
    ...
    trial = 'base'
    with open('data/final_result_{}_0606.json'.format(trial), 'w') as f:
        json.dump(result, f)
